...
test("UI CONTRACT 21: Numeric input validation contract (Section 39)", () => {
  const appJs = fs.readFileSync(path.join(process.cwd(), "public", "app.js"), "utf8");
...
test("UI CONTRACT 22: Renderer session serialization contract (Section 40)", () => {
  const appJs = fs.readFileSync(path.join(process.cwd(), "public", "app.js"), "utf8");
...
test("UI CONTRACT 23: Restart required UI contract (Section 41)", () => {
  const html = fs.readFileSync(path.join(process.cwd(), "public", "index.html"), "utf8");
  const appJs = fs.readFileSync(path.join(process.cwd(), "public", "app.js"), "utf8");
...
test("UI CONTRACT 24: rejectNumericEdit local serialization contract (Section 38)", () => {
  const appJs = fs.readFileSync(path.join(process.cwd(), "public", "app.js"), "utf8");
...
test("UI CONTRACT 25: every literal getElementById target in app.js exists in index.html (Section 5)", () => {
  const appJs = fs.readFileSync(path.join(process.cwd(), "public", "app.js"), "utf8");
  const html = fs.readFileSync(path.join(process.cwd(), "public", "index.html"), "utf8");
...
test("UI CONTRACT 26: every literal renderer command kind is accepted by sessionCommandSchema (Section 6)", () => {
  const appJs = fs.readFileSync(path.join(process.cwd(), "public", "app.js"), "utf8");
...
test("UI CONTRACT 27: single-villager quick actions use existing DOM IDs and supported bulk commands (Section 7)", () => {
  const appJs = fs.readFileSync(path.join(process.cwd(), "public", "app.js"), "utf8");
...
test("UI CONTRACT 28: Save Preview owns main OperationGate and renderer local busy state (Section 11)", () => {
...
test("UI CONTRACT 29: both world open paths mark storage restart state for typed fatal errors (Section 14)", () => {
  const ipcTs = fs.readFileSync(
    path.join(process.cwd(), "src", "main", "ipcHandlers.ts"),
    "utf8"
  );
...
test("UI CONTRACT 30: world:close failure reports restartRequired and renderer enters restart-required state (Section 18)", () => {
  const ipcTs = fs.readFileSync(
    path.join(process.cwd(), "src", "main", "ipcHandlers.ts"),
    "utf8"
  );
...
test("UI CONTRACT 31: typed post-save canonical reload error cannot be downgraded by cleanup close (Section 14)", () => {
  const ipcTs = fs.readFileSync(
    path.join(process.cwd(), "src", "main", "ipcHandlers.ts"),
    "utf8"
  );
...
test("UI CONTRACT 32: typed post-restore canonical reload error cannot be downgraded by cleanup close (Section 15)", () => {
  const ipcTs = fs.readFileSync(
    path.join(process.cwd(), "src", "main", "ipcHandlers.ts"),
    "utf8"
  );
...
test("UI CONTRACT 33: renderer handles committed save/restore with restartRequired before success/failure branching (Section 16)", () => {
  const appJs = fs.readFileSync(path.join(process.cwd(), "public", "app.js"), "utf8");
...
test("UI CONTRACT 37: MapReadBarrier post-drain health check and cleanup slice contracts (Section 12)", () => {
  const ipcTs = fs.readFileSync(path.join(process.cwd(), "src", "main", "ipcHandlers.ts"), "utf8");
  const regStart = ipcTs.indexOf("export function registerIpcHandlers(");
  assert.ok(regStart !== -1, "registerIpcHandlers definition not found");
...
test("UI CONTRACT 38: JSON export and re-import controls, IPC bindings, and transactional safety", () => {
  ...
  // Section 44: Main-process gating ordering assertions for villager:importJson
...
test("UI CONTRACT 40: BrowserWindow close shutdown ordering and disposal safety (Section 11)", () => {
