...
describe("Release Packaging & Secret Isolation", () => {
  ...
  it("release metadata includes the unofficial Minecraft notice in the product UI", () => {
    ...
    assert.ok(
      ...
      "Product UI must contain the unofficial Minecraft disclaimer"
    );

    assert.ok(
      html.includes('class="unofficial-product-notice"'),
      "Product UI disclaimer must use the visible notice element"
    );
  });
});
