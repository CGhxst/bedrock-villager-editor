...
An unofficial desktop application and CLI tool for Minecraft: Bedrock Edition (1.14 - 1.21.x) villager trades, professions, career tiers, enchantments, positions, and linked workstations.

---

## Supported Bedrock Versions

- **Supported Game Releases**: Minecraft: Bedrock Edition 1.14 through 1.21.x (Village & Pillage through current Tricky Trials releases).
- **World Storage**: Bedrock LevelDB save directories (SubChunk format versions 8 and 9).
- **Forward Compatibility**: Uses leaf-level NBT patching to preserve unknown tags and components. If an unreleased future version alters the underlying schema, the editor fails closed to protect save files from corruption.
