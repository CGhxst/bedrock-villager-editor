 ...
 *
 * Surface material coloring intentionally samples primary storage layer 0. Additional
 * Bedrock storage layers are not composited here; biome/water fallback remains available.
 */
export async function decodeSubChunkSurfaceBlocks(
  ...
  try {
    ...
    const storageCount = val[1];
    if (storageCount === undefined || storageCount < 1) {
      return null;
    }
    ...
    if (version === 9) {
      ...
      const storedY = val.readInt8(offset);
      if (storedY !== subChunkY) {
        return null;
      }
      offset += 1;
    }
    ...
    const isRuntime = (storageHeader & 1) === 1;
    const bitsPerBlock = storageHeader >> 1;

    // Persisted LevelDB chunks store NBT palettes (isRuntime === 0)
    // Runtime/network palette flag => return null visual fallback
    if (isRuntime) return null;

    const ALLOWED_BLOCK_BIT_WIDTHS: readonly number[] = [
      1,
      2,
      3,
      4,
    ...
    if (!ALLOWED_BLOCK_BIT_WIDTHS.includes(bitsPerBlock)) {
      return null;
    }
    ...
    if (offset + wordBytes + 4 > val.length) {
      return null;
    }

    const wordsOffset = offset;
    offset += wordBytes;

    const paletteSize = val.readUInt32LE(offset);
    offset += 4;
    ...
    for (let p = 0; p < paletteSize; p += 1) {
      ...
      const res = await nbt.parse(
        val.subarray(offset),
        "little"
      );

      const consumed = res.metadata?.size;
      const name = res.parsed.value?.name?.value;

      if (
        !Number.isInteger(consumed) ||
        consumed <= 0 ||
        offset + consumed > val.length ||
        typeof name !== "string" ||
        name.length === 0
      ) {
        return null;
    ...
    // Validate all logical 4096 packed indices after the palette is known
    const mask = (1 << bitsPerBlock) - 1;
    for (let idx = 0; idx < 4096; idx += 1) {
      const wordIdx = Math.floor(idx / blocksPerWord);
      const bitOffset = (idx % blocksPerWord) * bitsPerBlock;
  ...
  for (let localChunkZ = 0; localChunkZ < WORLD_MAP_TILE_CHUNKS; localChunkZ += 1) {
    for (let localChunkX = 0; localChunkX < WORLD_MAP_TILE_CHUNKS; localChunkX += 1) {
      ...
      for (const subChunkY of subChunkYSet) {
        const subKey = createSubChunkDataKey(
          chunkX,
          chunkZ,
          dimensionId,
          subChunkY
        ...
        // CRITICAL: LevelDB read failures are storage failures, not visual decode fallbacks.
        // Let db.get() reject so world:mapTile can preserve restart/quarantine handling.
        ...
        // decodeSubChunkSurfaceBlocks() is a non-throwing visual decoder. Unsupported or
        // malformed SubChunk bytes return null and fall back to biome/elevation rendering.
        const decoded = await decodeSubChunkSurfaceBlocks(
          subVal,
          chunk.data.heights,
          subChunkY
        );
